...
#import "ImageWithCache+More.h"

@interface ImageWithCache ()
...
@implementation ImageWithCache (More)

@synthesize theState=_theState,internetOperation=_internetOperation,delegate=_delegate;


+(id)imageWithUrl:(NSString *)imageUrl andProduct:(Product *)product context:(NSManagedObjectContext *)managedObjectContext{
    
    // Create Entity
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"ImageWithCache" inManagedObjectContext:managedObjectContext];
    
    // Initialize Record
    ImageWithCache *newImage = (ImageWithCache *)[[NSManagedObject alloc] initWithEntity:entity insertIntoManagedObjectContext:managedObjectContext];
    
    newImage.imageUrl=imageUrl;
    newImage.product=product;
    newImage.theState=ImageStateNew;
    newImage.internetOperation=nil;
...
-(UIImage *)retrieveImage:(id)sender fromCache:(NSCache *)cache atIOQueue:(NSOperationQueue *)ioQueue  atInternetQueue:(NSOperationQueue *)internetQueue{
    ...
    if (cachedImage) {
        ...
        if (isImageExists) {
            ...
            //避免retain cycle
            __weak ImageWithCache *wSelf=self;
