...
@implementation ImageWithCacheMore
...
+(id)imageWithUrl:(NSString *)imageUrl andProduct:(Product *)product context:(NSManagedObjectContext *)managedObjectContext{
    ...
    // Initialize Record
    ImageWithCacheMore *newImage = (ImageWithCacheMore *)[[NSManagedObject alloc] initWithEntity:entity insertIntoManagedObjectContext:managedObjectContext];
...
-(UIImage *)retrieveImage:(id)sender fromCache:(NSCache *)cache atIOQueue:(NSOperationQueue *)ioQueue  atInternetQueue:(NSOperationQueue *)internetQueue{
    ...
    if (cachedImage) {
        ...
        if (isImageExists) {
            ...
            //避免retain cycle
            __weak ImageWithCacheMore *wSelf=self;
