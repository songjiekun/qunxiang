...
@class ImageWithCacheMore;

@protocol ImageWithCacheMoreDelegate <NSObject>

//代理将更新UI
-(void)refreshImage:(ImageWithCacheMore *)sender withImage:(UIImage *)newImage;
...
/**
 ImageWithCacheMore是一个图片载入的类
 分三步来载入图片
 1.检查nscache中是否有内存缓存
 2.检查磁盘中是否有文件已经存在
 3.从互联网下载图片
 **/
@interface ImageWithCacheMore : ImageWithCache

//图片载入状态
@property (nonatomic,readonly) ImageState theState;

@property (nonatomic,weak) id <ImageWithCacheMoreDelegate> delegate;
